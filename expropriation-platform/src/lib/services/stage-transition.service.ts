...
export async function createStageNotifications(
  ...
  stage: CaseStage,
  user: UserWithRoleAndDepartment,
  caseData: { fileNumber: string; title: string }
) {
  try {
    ...
    logger.error('Error creating stage notifications:', error);
    // Don't throw error, as this is not critical for the main operation
  }
}
...
export async function createReturnNotifications(
  caseId: string,
  returnData: StageTransitionData,
  user: UserWithRoleAndDepartment,
  caseData: { fileNumber: string }
) {
  try {
    ...
    const notifications = usersToNotify.map(recipient => ({
      ...
      recipientId: recipient.id,
      priority: returnData.priority || 'high',
      sendEmail: true,
      metadata: {
        caseFileNumber: caseData.fileNumber,
